...
public class train : MonoBehaviour {
    ...
    void Update()
    {
        if (show)
        {
            jamimage.valueCurrent++;
            if (jamimage.valueCurrent == 100)
            {
        ...
        if (close)
        {
            ...
            {
                jamimage.valueCurrent--;
                if (jamimage.valueCurrent == 0)
                {
                    taihon.GetComponent<Text>().text = "僕：\n　　何が！強くなった！";
                    float fortitude = (Random.Range(1f, 4f));
                    int exp = 10;
                    close = false;
                    count = 0;
                    pro.Exp += exp / ((pro.level + 1) / 2);
                    pro.fortitude += fortitude;
                    date.SendMessage("onemonth");
                    pro.SendMessage("refresh");
                    Hpro.SendMessage("refresh");
                    infortext.text = "鍛える結果\n忍耐力：+" + (int)fortitude + "\nEXP：+10";
